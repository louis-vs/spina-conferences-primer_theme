    ...
    module PrimerTheme
      class AssetHelperTest < ActionView::TestCase
        ...
        test 'does not resize all variant options' do
          assert_equal({
                         main_app.url_for(Spina::Image.first.file.variant(resize_to_limit: [200, 200],
                                                                          rotate: 90)) => '1x',
                         main_app.url_for(Spina::Image.first.file.variant(resize_to_limit: [400, 400],
                                                                          rotate: 90)) => '2x',
                         main_app.url_for(Spina::Image.first.file.variant(resize_to_limit: [600, 600],
                                                                          rotate: 90)) => '3x',
                         main_app.url_for(Spina::Image.first.file.variant(resize_to_limit: [800, 800],
                                                                          rotate: 90)) => '4x'
                       },
